...
class BodyFatPrediction(models.Model):
    """Stores a body fat percentage prediction."""
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='bodyfat_predictions')
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, null=True, blank=True, related_name='bodyfat_predictions')
    predicted_percent = models.FloatField(help_text='Predicted body fat percentage')
    uncertainty = models.FloatField(null=True, blank=True, help_text='Optional uncertainty estimate')
    inputs = models.JSONField(blank=True, null=True, help_text='Original input features')
    model_version = models.CharField(max_length=50, default='v1')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        owner = self.user.username if self.user else 'anonymous'
        return f"BodyFat {self.predicted_percent:.1f}% for {owner} ({self.created_at.date()})"
...
class GymRecommendation(models.Model):
    """Stores a saved workout recommendation generated by the GymRecommender."""
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='gym_recommendations')
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, null=True, blank=True, related_name='gym_recommendations')
    recommended_workout = models.CharField(max_length=100)
    confidence = models.FloatField(help_text='Probability confidence (0-1)')
    inputs = models.JSONField(blank=True, null=True, help_text='Original input features used for recommendation')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        owner = self.user.username if self.user else 'anonymous'
        return f"Recommendation for {owner}: {self.recommended_workout} ({self.confidence:.2f})"
